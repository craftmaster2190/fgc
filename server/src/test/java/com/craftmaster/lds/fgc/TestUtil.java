...
public class TestUtil {
  ...
  /**
   * This method tests the associated read and/or write methods for all "properties" of the
   * testObject, except the passed in field names
   *
   * @param testObject Object on which to call the getters and/or setters
   * @param propertyNames Array (varargs) of property names whose associated read and/or write
  ...
  /**
   * This method tests the associated read and/or write methods for the passed in "property" names,
   * of the testObject
   *
   * @param testObject Object on which to call the read and/or write methods
   * @param propertyNames Array (varargs) of field names whose associated getters and/or setters
   *     SHOULD be tested
   * @throws throws a runtime exception if the test fails
   */
  ...
  /**
   * This method tests the read and/or write methods for all "properties" on the given testObject.
   * This only attempts to test getters and setters of the class or instance passed in, and not the
   * getters and setters of any superclasses. However, if there is a getter but not an associated
   * setter, or vice versa, it will search parent classes for an associated match to create a true
   * property.
   *
   * <p>The test is accomplished with the following rules: If there is a getter and a setter (a true
   * property) it tests that the getter returns what was set on the setter If there is a getter and
   * a field (no setter) it will test that what is set on the field is returned by the getter
   * &#160;&#160;&#160;If the field is final the field value is tested against the getter If there
   * is a setter and a field (no getter) it will make sure that value passed to the setter is the
   * value of the field
   *
   * <p>If there is a getter, but no setter, all parent classes will be searched for a matching
   * setter. If there is a setter, but no getter, all parent classes will be searched for a matching
   * getter.
   *
   * @param testObject Object on which to call the read and/or write methods
   * @throws throws a runtime exception if the test fails
  ...
  /**
   * This method determines if the passed in object if of type Class, and if it is, it attempt to
   * instantiate an instance of the class with whic to test the properties.
   *
   * @param testObject Object passed in by user which is an instance or from which we create an
   *     instance.
   * @return The instantiated object if it is a class, or the object passed in if not.
   */
  ...
  /**
   * Simple utility method that can simplify aquiring a private member. Useful in test assertions.
   *
  ...
  public static Object getField(Object target, String fieldName) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error attempting to get value of field '"
              + fieldName
              + "' on object of type'"
  ...
  public static void testEquals(Object testObject1, Object testObject2) {
    try {
      ...
      if (!((Boolean) equalsMethod.invoke(testObject1, testObject1))) {
        throw new RuntimeException(
            "The two instances of the same class should be equal, but they were not.");
      }
  ...
  protected static void test(Object testObject, String[] propertyNames, Boolean include) {
    ...
    for (String propertyName : keys) {
      ...
      if (propertyInfo.getWriteMethods().size() <= 0) {
        testReadAndOrWriteMethods(
            testObject, propertyInfo.getReadMethod(), null, propertyInfo.getField(), propertyName);
      } else {
  ...
  /**
   * This method collects all of the properties (meaning readMethod, writeMethods, associated field)
   * in the given test object and adds them to a map of PropertyInfo objects. This is done by
   * looking for all methods that start with a get or is, and have a return type not equal to void
   * and no parameters, or methods beginning with set that have one parameter and a return type of
   * void.
   *
   * @param testObject
   * @param propertyNames List of properties to include, or exclude from the test
   * @param include Whether to include, or exclude the names in <code>propertyNames</code>. If
   *     include is null, all found properties are tested.
   * @return A map of PropertyInfo objects with the property as a key. For example, if a method
   *     getSomething was found in the testObject, there would be a PropertyInfo object with
   *     readMethod set to getSomething() Method and the key for the propertyInfo Object would be
   *     "something".
   */
  protected static Map<String, PropertyInfo> collectProperties(
      Object testObject, List<String> propertyNames, boolean include) {
    ...
    Class<?> testClass =
        isCglibProxyClass(testObject.getClass())
            ? testObject.getClass().getSuperclass()
            : testObject.getClass();
    ...
    for (Method method : testClass.getDeclaredMethods()) {
      ...
      if ((methodName.startsWith("get")
        ...
        if ((include && propertyNames.contains(propertyName))
            || (!include && !propertyNames.contains(propertyName))) {
          PropertyInfo propertyInfo = propertyMap.get(propertyName);
  ...
  /**
   * This method goes through all of the properties collected (i.e. all methods for which either a
   * getter and/or setter was found), and if there is a getter and no associated setter, or a setter
   * and no associated getter is searches for one in the test objects superclasses
   *
   * @param propertyMap
   * @param testObject
   * @return
   */
  protected static Map<String, PropertyInfo> fillInHierarchicalInfo(
      Map<String, PropertyInfo> propertyMap, Object testObject) {
    Set<String> keys = propertyMap.keySet();
    for (String propertyName : keys) {
      PropertyInfo propertyInfo = propertyMap.get(propertyName);
      ...
      if (propertyInfo.getReadMethod() == null) {
        propertyInfo.setReadMethod(
            findDeclaredMethodInHierarchy(testObject, "get" + upperCaseFirstLetter(propertyName)));
        if (propertyInfo.getReadMethod() == null) {
          propertyInfo.setReadMethod(
              findDeclaredMethodInHierarchy(testObject, "is" + upperCaseFirstLetter(propertyName)));
        }
        ...
        Method method =
            findDeclaredMethodInHierarchy(
                ...
                propertyInfo.getReadMethod() != null
                    ? propertyInfo.getReadMethod().getReturnType()
                    : null);
        if (method != null) {
          propertyInfo.addWriteMethod(method);
        }
  ...
  protected static void testReadAndOrWriteMethods(
      Object testObject, Method readMethod, Method writeMethod, Field field, String propertyName) {
    ...
    if (readMethod == null && writeMethod == null) {
      throw new RuntimeException(
          MessageFormat.format("Could not find method for property named {0}.", propertyName));
    }
   ...
   * @throws Exception
   */
  protected static void testProperty(
      Object testObject, Method readMethod, Method writeMethod, String propertyName) {
    ...
    try {
      ...
      if (proxy.equals(readMethod.invoke(testObject))) {
        logger.log(
            Level.INFO,
            MessageFormat.format(
                "TEST OF **** {0} AND {1} **** SUCCEEDED.",
                readMethod.getName(), writeMethod.getName()));
      } else {
        throw new RuntimeException(
            MessageFormat.format("Test for property {0} failed.", propertyName));
      }
  ...
  private static void handleInvocationTargetException(
      String propertyName, InvocationTargetException e) {
    if (e.getTargetException() instanceof RuntimeException) {
      throw (RuntimeException) e.getTargetException();
    } else {
      throw new RuntimeException(
          "Exception thrown while invoking method for property: " + propertyName,
   ...
   * @throws Exception
   */
  protected static void testReadOnly(
      Object testObject, Method readMethod, Field field, String propertyName) {
    try {
      readMethod.setAccessible(true);
      ...
      if ((field.getModifiers() & Modifier.FINAL) != Modifier.FINAL) {
        ...
        if (proxy.equals(readMethod.invoke(testObject))) {
          logger.log(
              Level.INFO,
              MessageFormat.format(
                  "TEST OF **** direct set AND {0} **** SUCCEEDED.", readMethod.getName()));
        } else {
          field.set(testObject, saveOffValue);
          throw new RuntimeException(
              MessageFormat.format(
                  "Test for get{0} or is{1} on class {2} failed.",
        ...
        if ((getterValue != null && getterValue.equals(fieldValue))
            || (getterValue == null && fieldValue == null)) {
          logger.log(
              Level.INFO,
              MessageFormat.format(
                  "TEST OF **** final member variable AND {0} **** SUCCEEDED.",
                  readMethod.getName()));
        } else {
          throw new RuntimeException(
              MessageFormat.format(
                  "Test for get{0} or is{1} and final member variable on class {2} failed.",
                  upperCaseFirstLetter(propertyName),
                  upperCaseFirstLetter(propertyName),
                  testObject));
        }
   ...
   * @throws Exception
   */
  protected static void testWriteOnly(
      Object testObject, Method writeMethod, Field field, String propertyName) {
    try {
      writeMethod.setAccessible(true);
      ...
      if (proxy.equals(field.get(testObject))) {
        logger.log(
            Level.INFO,
            MessageFormat.format(
                "TEST OF **** {0} AND direct get **** SUCCEEDED.", writeMethod.getName()));
      } else {
        throw new RuntimeException(
            MessageFormat.format(
                "Test for {0} on class {1} failed.", writeMethod.getName(), testObject));
      }
      field.set(testObject, saveOffValue);
    } catch (InvocationTargetException e) {
      handleInvocationTargetException(propertyName, e);
  ...
  /**
   * Compiles a list of fields found in the object passed in, or any superclasses of the object.
   *
   * @param testObject The object whose fields and fields of its superclass should be retrieved
   * @return List<Field> of all the fields of the class and all of its superclasses
   */
  ...
  protected static Method findDeclaredMethodInHierarchy(
      Object testObject, String methodName, Class<?>... parameterTypes) {
    Method found = null;
    Class<?> objectClass = testObject.getClass();
    do {
      ...
      try {
        ...
        logger.log(
            Level.FINE,
            MessageFormat.format(
                "No accessor named {0} was found in class {1}.",
                methodName, testObject.getClass()));
      }
      objectClass = objectClass.getSuperclass();
    } while (objectClass != null);
  ...
  /**
   * Converts the first letter of the given string to upper case. For example: "propertyName" should
   * be returned as "PropertyName"
   *
  ...
  /**
   * This method creates a proxy object for the given type using a no-arg constructor.
   *
   * @param type Class type of the object or primitive to be proxied
   * @return The proxied object or wrapped primitive
   * @throws IllegalAccessException
  ...
  private static Object createCgLibProxy(Class<?> type) {
    throw new RuntimeException(
        format(
            "We don't want to invoke cgLib for type: %s, lets figure this out",
